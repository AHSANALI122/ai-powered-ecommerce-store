...
- **Checkout's verification gate is policy; its session gate is not.** `REQUIRE_VERIFIED_EMAIL_FOR_CHECKOUT` (default `true`, F1 DoD) is read through `canCheckOut`/`checkoutRequiresVerifiedEmail` in `src/lib/auth/current-user.ts`, and all four checkout entry points read it there — the cart button, `/checkout`, `/api/checkout` and `/api/checkout/quote`. Enforcing it in three and not the fourth gives a button that 403s after it is pressed. Setting it `false` is allowed in production (it warns at boot) because, unlike `EMAIL_DRIVER="log"`, it does less rather than pretending to work. **Review submission is not covered** — `requireVerifiedUser`/`requireApiVerifiedUser` stay unconditional there, because that gate is anti-spam, not contactability. A signed-in session is never optional either way: an order needs an owner to filter by (SEC-23).
